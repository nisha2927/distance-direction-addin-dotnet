...
var mo = dojoDeclare([dojoStateful], {
        ...
        if (itm.length === 1) {
          ...
          this.util.getXYNotation(sortedInput, itm[0].conversionType).then(dojoLang.hitch(this, function (r) {
            if (r.length <= 0 || (!r[0][0] && r[0][0] !== 0)) {
              this.hasError = true;
              this.valid = false;
              this.message = this.nls.invalidCoordinateMessage;
              ...
              this.coordinateEsriGeometry = new EsriPoint(r[0][0], r[0][1],
                new EsriSpatialReference({ wkid: 4326 }));
              this.message = '';
              this.inputTypeDef.resolve(this);
            }
  ...
  processCoordTextInput: function (withStr, asType, testingMode) {
    ...
    var northSouthPrefix, northSouthSuffix, eastWestPrefix,
      eastWestSuffix, latDeg, longDeg, latMin, longMin, latSec, longSec;
    ...
    if (prefixSuffixError) {
      if (!testingMode) {
        new JimuMessage({ message: this.nls.prefixSuffixMessage });
      }
    }
