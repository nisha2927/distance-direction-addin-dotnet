  ...
  var clz = dojoDeclare([drawFeedback], {
    ...
    */
    onCreateManualEllipse: function (majorLength,minorLength,orientationAngle,centerPoint) {
        ...
        var lengthInMeters = this.coordTool.convertToMeters(majorLength, this.lengthUnit);
        ...
        lengthInMeters = this.coordTool.convertToMeters(minorLength, this.lengthUnit);
    ...
    */
        ...
        case esriDraw.POLYLINE:
          switch(this._points.length) {
            case 1:
              ...
              if (tooltip) {
                tooltip.innerHTML = jimuUtils.sanitizeHTML(this.nls.majorAxisTooltipMessage);
              }
              break;

            case 2:
              if (this._tooltip) {
                this._tooltip.innerHTML = jimuUtils.sanitizeHTML(this.nls.minorAxisTooltipMessage);
              }
              break;
            case 3:
              this._onDoubleClickHandler();
    ...
    _onMouseMoveHandler: function (evt) {
      ...
      if (this._points.length === 1) {
        ...
        var majorUnitLength = this.coordTool.convertMetersToUnits(this.majorAxisLength, this.lengthUnit);
        ...
        if (this._minGraphic !== null){
          ...
          var minorUnitLength = this.coordTool.convertMetersToUnits(this.minorAxisLength[0], this.lengthUnit);
    ...
    _onDoubleClickHandler: function () {
      if (this._points.length >= 3)  {
        ...
        elipseGeom.geometry = esriGeometryEngine.rotate(ellipse,
          this.convertAngle(this.orientationAngle),this._majGraphic.geometry.getPoint(0,0));

        elipseGeom = dojoLang.mixin(elipseGeom, {
          majorAxisLength: this.coordTool.convertMetersToUnits(this.majorAxisLength, this.lengthUnit),
          minorAxisLength: this.coordTool.convertMetersToUnits(this.minorAxisLength, this.lengthUnit),
          angle: this.angle,
          drawType: 'ellipse',
